...
def ensure_nltk_resources():
    ...
    for resource in NLTK_RESOURCES:
        try:
            ...
            nltk.download(
                resource.split("/")[1], quiet=True, timeout=10
            )  # Download the resource with a timeout
...
class EnhancedTextPreprocessor:
    ...
    def preprocess(self, text: str) -> str:
        ...
        cleaned = self.regex_patterns["url"].sub(
            "", cleaned
        )  # Remove URLs from the text
        cleaned = self.regex_patterns["email"].sub(
            "", cleaned
        )  # Remove email addresses from the text
        cleaned = self.regex_patterns["special_chars"].sub(
            " ", cleaned
        )  # Remove special characters from the text
        cleaned = (
            self.regex_patterns["whitespace"].sub(" ", cleaned).strip()
    ...
    def tokenize_batch(self, texts: List[str]) -> List[List[str]]:
        ...
        memory_per_process = self.config.get(
            "memory_per_process", 50 * 1024 * 1024
        )  # Default to 50MB per process
        safe_memory_per_process = (
            available_memory // (n_process + 1) if n_process > 0 else available_memory
        )
...
class AdvancedKeywordExtractor:
    ...
    def _is_in_context(self, keyword: str, text: str) -> bool:
        ...
        for section_text in sections.values():  # Iterate through the sections
            if (
                keyword.lower() in section_text.lower()
            ):  # Check if the keyword is in the section
        ...
        for sent in doc.sents:  # Iterate through the sentences in the text
            if (
                keyword.lower() in sent.text.lower()
...
class ATSOptimizer:
    ...
    def _init_categories(self):
        ...
        def calculate_category_vector(category, terms):
            ...
            """
            vectors = [
                self._get_term_vector(term)
                for term in terms
                if self._get_term_vector(term).any()
            ]  # Get term vectors
    ...
    def _calculate_scores(self, dtm, feature_names, keyword_sets, job_descriptions):
        ...
        for row, col, tfidf in zip(dtm_coo.row, dtm_coo.col, dtm_coo.data):
            ...
            score = self.config["weighting"]["tfidf_weight"] * tfidf + self.config[
                "weighting"
            ]["frequency_weight"] * np.log1p(freq)
    ...
    def _create_tfidf_matrix(self, texts, keyword_sets):
        ...
        if len(vectorizer.get_feature_names_out()) == 5000:
            logger.warning(
                "TF-IDF vocabulary truncated to 5000 features. Consider increasing max_features."
            )
    ...
    def _analyze_jobs_chunked(
        ...
        num_workers = min(
            self.config.get("max_workers", psutil.cpu_count()), len(chunks)
        )
        results = []
        with Pool(processes=num_workers) as pool:
            for chunk_result in pool.imap_unordered(self._process_chunk, chunks):
                ...
                if psutil.virtual_memory().percent > self.config.get(
                    "memory_threshold", 70
                ):
                    chunk_size = self._calculate_chunk_size(job_descriptions)
                    logger.warning(
                        f"Memory usage is high. Adjusting chunk size to: {chunk_size}"
                    )
                    break
        if not results:
            return pd.DataFrame(), pd.DataFrame()
    ...
    def _process_chunk(
        ...
        try:
            ...
            results = self._calculate_scores(
                dtm, features, keyword_sets, list(chunk.keys())
            )
    ...
    def _analyze_jobs_internal(
        ...
        results = self._calculate_scores(
            dtm, feature_names, keyword_sets, list(job_descriptions.keys())
        )
        df = pd.DataFrame(results)
        summary = (
            df.groupby("Keyword")
    ...
    def _validate_description(self, desc) -> ValidationResult:
        ...
        cleaned_desc = re.sub(
            r"[\x00-\x1F\x7F-\x9F]", "", cleaned_desc, flags=re.UNICODE
        )
        cleaned_desc = re.sub(
